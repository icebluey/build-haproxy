...
_build_zlib() {
    ...
    cd /tmp/zlib
    _strip_files
    install -m 0755 -d usr/lib64/haproxy/private
    cp -af usr/lib64/*.so* usr/lib64/haproxy/private/
...
_build_haproxy() {
    ...
    rm -f haproxy-*.tar*
    cd haproxy*
    LDFLAGS='' ; LDFLAGS="${_ORIG_LDFLAGS}"' -Wl,-rpath,/usr/lib64/haproxy/private' ; export LDFLAGS
    ...
    /usr/local/private/liblz4.so* \
    /usr/local/private/libzstd.so* \
    /usr/local/private/libedit.so* \
    /usr/local/private/libpcre2* \
    /usr/local/private/libssl.so* \
    /usr/local/private/libcrypto.so* \
    usr/lib64/haproxy/private/
    rm -fr var
    echo
    sleep 2
    tar -Jcvf /tmp/haproxy-"${_haproxy_ver}"-1.el8.x86_64.tar.xz *
    echo
    sleep 2
    cd /tmp
    openssl dgst -r -sha256 haproxy-"${_haproxy_ver}"-1.el8.x86_64.tar.xz | sed 's|\*| |g' > haproxy-"${_haproxy_ver}"-1.el8.x86_64.tar.xz.sha256
    rm -fr "${_tmp_dir}"
    rm -fr /tmp/haproxy
    /sbin/ldconfig
}
...
rm -fr /usr/lib64/haproxy/private
